...
export function CaseRuleAuditView({ audit, targetContentIds }: CaseRuleAuditViewProps) {
  ...
  return (
    <details className="case-rule-audit" open>
      <summary>
      ...
      <p className="audit-disclaimer">
        Read-only snapshot from case version {audit.contentVersion}. These are current game values,
        not medically approved recommendations.
      </p>
                ...
                <dl>
                  ...
                  {rule.fulfillment.map((method) => (
                    <div key={`${rule.id}.${method.actionId}`}>
                      <dt>Current cost</dt>
                      <dd>
                        {method.methodLabel} · {words(method.methodKind)} · {method.operatingCost}{' '}
                        pts
                      </dd>
                    </div>
                  ))}
                  {rule.conditionalEffects.map((effect) => (
                    <div key={`${rule.id}.${effect.pathwayId}`}>
                      <dt>{effect.safetyCritical ? 'Safety-critical path' : 'Path modifier'}</dt>
                      <dd>
            ...
            {focused.treatmentPathways.map((rule) => (
              <article className="audit-row" key={rule.id}>
                <header>
                  <strong>{rule.label}</strong>
                  <span>
                    {rule.accepted ? 'accepted' : 'not accepted'} · {words(rule.grade)} · workup par{' '}
                    {rule.workupCostPar} pts
                  </span>
                </header>
                <p>
                  <b>When:</b> {rule.condition}
                ...
                {rule.conditionalRequirements.map((requirement) => (
                  <p key={`${rule.id}.${requirement.objectiveId}`}>
                    ...
                    {requirement.objectiveLabel} · met {signed(requirement.pointsIfMet)} · missing{' '}
                    {signed(requirement.pointsIfMissing)}
                  </p>
                ))}
                <p>{rule.explanation}</p>
                <ReviewLine review={rule.review} />
      ...
      <details className="audit-treatment-menu">
        ...
        <ul>
          {audit.referenceSolutions.map((reference) => (
            <li key={reference.id}>
              <strong>{reference.label}</strong> · {words(reference.kind)} · {reference.actionCount}{' '}
              investigations
