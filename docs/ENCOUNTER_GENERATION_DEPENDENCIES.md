...
| Latent propositions and patient-scene evidence | Approved Boolean proposition/source-claim/dependency architecture; compatibility source-scoped findings                              | Versioned point-free proposition, evidence, dependency-group, and belief-appraisal schemas; no generic Bayesian or credibility engine           | `ticket.schema.patient-state.latent-proposition-evidence-foundation`                                               |
...
3. `ticket.schema.patient-state.latent-proposition-evidence-foundation`
