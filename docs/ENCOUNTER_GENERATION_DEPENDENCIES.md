...
The canonical finding-definition boundary now owns 27 identity-only, medically unreviewed
definitions, typed outcome semantics, explicit unresolved states, and contributor provenance. The
approved 37-candidate audit added only unambiguous identities: duration/ordinal values and eight
...
| General psychiatry findings              | 27 identity-only canonical definitions, many compatibility IDs, and 40 neutral reveal actions                                | Resolve the bounded semantic/value collisions one at a time; add substance, MSE, and physical owners without probabilities or scoring           | `ticket.catalog.findings.general-psychiatry-seed` (resolved 2026-07-28); first collision: `ticket.catalog.findings.diminished-interest-pleasure-boundary` |
...
2. `ticket.catalog.findings.general-psychiatry-seed` (resolved 2026-07-28; 27 definitions)
   - next bounded review: `ticket.catalog.findings.diminished-interest-pleasure-boundary`
