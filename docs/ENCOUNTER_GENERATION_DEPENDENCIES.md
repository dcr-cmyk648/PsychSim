...
The first canonical finding-definition boundary now owns one identity-only depressed-mood seed,
typed outcome semantics, explicit unresolved states, and contributor provenance. It intentionally
does not migrate or normalize the other case-local occurrences.
...
| Atomic findings                          | Case-local compatibility plus versioned canonical definition, resolved-value, contributor-provenance, and projection schemas; one identity-only seed | Technical boundary complete; broader identities and runtime compilation remain in their separate rows/tickets                                   | `ticket.catalog.findings.canonical-definition-boundary` (resolved 2026-07-28)                                      |
| General psychiatry findings              | Many repeated finding IDs and 40 neutral reveal actions                                                                                              | Normalize reused history, symptom, function, safety, substance, MSE, and physical concepts without adding probabilities or scoring              | `ticket.catalog.findings.general-psychiatry-seed`                                                                  |
...
2. `ticket.catalog.findings.general-psychiatry-seed`
...
6. missing rule or route coverage becomes a nonblocking diagnostic and ticket rather than an
   invented default or patient deletion;
7. literal same-scope contradictions retry deterministically or quarantine;
8. the same recipe, clinic state, seed, and content versions reproduce the same resolved patient,
   encounter, rubric, score trace, and settlement;
9. current `CaseBlueprint` attempts still replay unchanged through an explicit compatibility or
   migration path; and
10. Player, Reviewer, and Developer bundle boundaries remain intact.
