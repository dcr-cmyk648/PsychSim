...
def worktree_snapshot(root: Path) -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "project_state_sha256": (
            sha256_bytes(project_state.read_bytes()) if project_state.exists() else None
        ),
    }
...
def snapshot_differences(
    ...
    labels = {
        ...
        "project_state_sha256": "PROJECT_STATE.md",
    }
    return [
        labels[key]
        for key in labels
    ...
    if current_relation == "canonical":
        if value.get("status") == "needs-reconciliation":
            ...
            return value, additional_context(
                ...
                "and PROJECT_STATE.md, report discrepancies, and do not edit until the "
